export function generateGenericSteps(algorithmId, input = {}) {
  ...
  else if (algorithmId === "longest-consecutive-sequence") {
    const nums = [100, 4, 200, 1, 3, 2];
    steps.push({ line: 1, description: "🔍 Longest Consecutive Sequence! Array: [" + nums.join(", ") + "]", state: { array: [...nums] } });
    const numSet = new Set(nums);
    let longest = 0;
    steps.push({ line: 2, description: "Store all elements in a hash set to enable O(1) lookups", state: { array: [...nums], set: nums.join(", ") } });
    for (let i = 0; i < nums.length; i++) {
      const n = nums[i];
      const isStart = !numSet.has(n - 1);
      steps.push({ line: 3, description: "Check if " + n + " is the start of a sequence (does set contain " + (n - 1) + "? " + (!isStart ? "Yes" : "No") + ")", state: { array: [...nums], check_idx: i } });
      if (isStart) {
        let length = 0;
        while (numSet.has(n + length)) {
          length++;
          steps.push({ line: 4, description: "Sequence grows: [" + Array.from({ length }, (_, idx) => n + idx).join(", ") + "] (length: " + length + ")", state: { array: [...nums], check_idx: i, sequence_length: length } });
        }
        const oldLongest = longest;
        longest = Math.max(longest, length);
        steps.push({ line: 5, description: "Sequence of length " + length + " finished. " + (length > oldLongest ? "🎉 New longest sequence = " + longest + "!" : "Keep longest = " + longest), state: { array: [...nums], check_idx: i, sequence_length: length, max_length: longest } });
      }
    }
    steps.push({ line: 8, description: "✅ Completed. Longest consecutive sequence length is " + longest, state: { array: [...nums], result: longest } });
  }

  // ── Min Heap Operations ────────────────────────────────────────────────────
  else if (algorithmId === "min-heap") {
    const arr = [10, 15, 30, 40, 50, 100];
    const insertVal = 8;
    steps.push({ line: 1, description: "📥 Min Heap Insertion! Insert val " + insertVal + " into heap: [" + arr.join(", ") + "]", state: { array: [...arr], heap_size: arr.length } });
    arr.push(insertVal);
    let idx = arr.length - 1;
    steps.push({ line: 3, description: "Add " + insertVal + " at the end of the heap array (index " + idx + "): [" + arr.join(", ") + "]", state: { array: [...arr], check_idx: idx, heap_size: arr.length } });
    
    while (idx > 0) {
      const parentIdx = Math.floor((idx - 1) / 2);
      steps.push({ line: 4, description: "Compare element arr[" + idx + "] = " + arr[idx] + " with parent arr[" + parentIdx + "] = " + arr[parentIdx], state: { array: [...arr], check_idx: idx, parent_idx: parentIdx } });
      if (arr[idx] < arr[parentIdx]) {
        [arr[idx], arr[parentIdx]] = [arr[parentIdx], arr[idx]];
        steps.push({ line: 5, description: "Bubble Up: swap child (" + arr[parentIdx] + ") and parent (" + arr[idx] + ") since " + arr[parentIdx] + " < " + arr[idx], state: { array: [...arr], check_idx: parentIdx, swapped: true } });
        idx = parentIdx;
      } else {
        steps.push({ line: 6, description: "Heap property satisfied (child " + arr[idx] + " ≥ parent " + arr[parentIdx] + "). Bubble up ends.", state: { array: [...arr] } });
        break;
      }
    }
    steps.push({ line: 8, description: "✅ Insertion complete! Final Min Heap array: [" + arr.join(", ") + "]", state: { array: [...arr] } });
  }

  // ── Edit Distance ─────────────────────────────────────────────────────────
  else if (algorithmId === "edit-distance") {
    const word1 = input.word1 || "horse";
    const word2 = input.word2 || "ros";
    const m = word1.length;
    const n = word2.length;
    steps.push({ line: 1, description: "✍️ Edit Distance between "" + word1 + "" and "" + word2 + "". Min operations to convert word1 to word2.", state: { word1, word2 } });
    const dp = Array.from({ length: m + 1 }, () => new Array(n + 1).fill(0));
    
    for (let i = 0; i <= m; i++) dp[i][0] = i;
    for (let j = 0; j <= n; j++) dp[0][j] = j;
    steps.push({ line: 3, description: "Initialize DP table with base cases (empty string operations)", state: { matrix: dp.map(r => [...r]), word1, word2 } });
    
    for (let i = 1; i <= m; i++) {
      for (let j = 1; j <= n; j++) {
        steps.push({ line: 5, description: "Compare char1='" + word1[i-1] + "' (at index " + i + ") and char2='" + word2[j-1] + "' (at index " + j + ")", state: { matrix: dp.map(r => [...r]), row: i, col: j, word1, word2 } });
        if (word1[i - 1] === word2[j - 1]) {
          dp[i][j] = dp[i - 1][j - 1];
          steps.push({ line: 6, description: "Characters match ('" + word1[i-1] + "'). No operation needed: dp[" + i + "][" + j + "] = dp[" + (i-1) + "][" + (j-1) + "] = " + dp[i][j], state: { matrix: dp.map(r => [...r]), row: i, col: j, word1, word2 } });
        } else {
          const insert = dp[i][j - 1];
          const remove = dp[i - 1][j];
          const replace = dp[i - 1][j - 1];
          dp[i][j] = 1 + Math.min(insert, remove, replace);
          steps.push({ line: 8, description: "Mismatch. Choose min of Insert (" + insert + "), Delete (" + remove + "), Replace (" + replace + ") + 1 = " + dp[i][j], state: { matrix: dp.map(r => [...r]), row: i, col: j, word1, word2 } });
        }
      }
    }
    steps.push({ line: 10, description: "✅ Edit Distance completed! Minimum operations needed: " + dp[m][n], state: { matrix: dp.map(r => [...r]), word1, word2, result: dp[m][n] } });
  }
  ...
  // ── Longest Increasing Subsequence ────────────────────────────────────────
    ...
    const n = nums.length;
    steps.push({ line: 1, description: "📈 LIS! Find length of Longest Increasing Subsequence in [" + nums.join(", ") + "]", state: { array: [...nums] } });
    const dp = new Array(n).fill(1);
    steps.push({ line: 3, description: "Initialize DP array filled with 1s (each element is a subsequence of length 1)", state: { array: [...nums], dp: [...dp] } });
    for (let i = 0; i < n; i++) {
      for (let j = 0; j < i; j++) {
        steps.push({ line: 5, description: "Compare nums[i=" + i + "] (" + nums[i] + ") with nums[j=" + j + "] (" + nums[j] + ")", state: { array: [...nums], dp: [...dp], check_idx: i, compare_idx: j } });
        if (nums[i] > nums[j]) {
          const prev = dp[i];
          dp[i] = Math.max(dp[i], dp[j] + 1);
          steps.push({ line: 6, description: "nums[i] > nums[j] (" + nums[i] + " > " + nums[j] + "). Update dp[" + i + "] = max(dp[" + i + "], dp[" + j + "]+1) = max(" + prev + ", " + (dp[j] + 1) + ") = " + dp[i], state: { array: [...nums], dp: [...dp], check_idx: i, compare_idx: j } });
        }
      }
    }
    const maxLIS = Math.max(...dp, 0);
    steps.push({ line: 8, description: "✅ Longest Increasing Subsequence length is max(dp) = " + maxLIS, state: { array: [...nums], dp: [...dp], result: maxLIS } });
  }

  // ── Longest Common Prefix ─────────────────────────────────────────────────
  else if (algorithmId === "longest-common-prefix") {
    const strs = input.strs || ["flower", "flow", "flight"];
    steps.push({ line: 1, description: "🔤 Longest Common Prefix for: [" + strs.map(s => '"' + s + '"').join(", ") + "]", state: { words: [...strs] } });
    let prefix = "";
    if (strs.length > 0) {
      const first = strs[0];
      for (let i = 0; i < first.length; i++) {
        const char = first[i];
        let match = true;
        steps.push({ line: 3, description: "Check index " + i + ": character '" + char + "' from first word "" + first + """, state: { words: [...strs], prefix, check_idx: i, char } });
        for (let j = 1; j < strs.length; j++) {
          if (i === strs[j].length || strs[j][i] !== char) {
            match = false;
            steps.push({ line: 5, description: "Mismatch at word " + j + " ("" + strs[j] + ""). Common prefix ends.", state: { words: [...strs], prefix, check_idx: i, word_idx: j } });
            break;
          }
        }
        if (!match) break;
        prefix += char;
        steps.push({ line: 6, description: "Match found! Extended common prefix to: "" + prefix + """, state: { words: [...strs], prefix, check_idx: i } });
      }
    }
    steps.push({ line: 8, description: "✅ Completed Longest Common Prefix. Result: "" + prefix + """, state: { words: [...strs], prefix } });
  }

  // ── Rotate Array ──────────────────────────────────────────────────────────
  else if (algorithmId === "rotate-array") {
    const arr = input.arr || [1, 2, 3, 4, 5, 6, 7];
    const k = input.k || 3;
    steps.push({ line: 1, description: "🔄 Rotate Array by k = " + k + " places. Array: [" + arr.join(", ") + "]", state: { array: [...arr], k } });
    const actualK = k % arr.length;
    
    const reverse = (left, right, label) => {
      steps.push({ line: 3, description: "Reverse segment: " + label + " (indices [" + left + " .. " + right + "])", state: { array: [...arr], left, right } });
      while (left < right) {
        [arr[left], arr[right]] = [arr[right], arr[left]];
        left++;
        right--;
      }
      steps.push({ line: 3, description: "After reversing segment: [" + arr.join(", ") + "]", state: { array: [...arr] } });
    };
    
    reverse(0, arr.length - 1, "entire array");
    reverse(0, actualK - 1, "first k elements");
    reverse(actualK, arr.length - 1, "remaining elements");
    steps.push({ line: 5, description: "✅ Rotation complete! Final rotated array: [" + arr.join(", ") + "]", state: { array: [...arr] } });
  }

  // ── Trapping Rain Water ───────────────────────────────────────────────────
  else if (algorithmId === "trapping-rain-water") {
    const height = input.height || [0, 1, 0, 2, 1, 0, 1, 3, 2, 1, 2, 1];
    steps.push({ line: 1, description: "🌧️ Trapping Rain Water! Find total water trapped. Heights: [" + height.join(", ") + "]", state: { array: [...height] } });
    let l = 0, r = height.length - 1;
    let lMax = height[l], rMax = height[r];
    let totalWater = 0;
    steps.push({ line: 3, description: "Pointers initialized at l=0, r=" + r + ". lMax=" + lMax + ", rMax=" + rMax, state: { array: [...height], left: l, right: r, l_max: lMax, r_max: rMax, total_water: totalWater } });
    while (l < r) {
      if (lMax < rMax) {
        l++;
        ...
        totalWater += water;
        steps.push({ line: 5, description: "lMax < rMax. Shift left pointer to " + l + " (val " + height[l] + "). New lMax=" + lMax + ". Water at index " + l + " = " + water + ". Total trapped water = " + totalWater, state: { array: [...height], left: l, right: r, l_max: lMax, r_max: rMax, total_water: totalWater } });
      } else {
        r--;
        rMax = Math.max(rMax, height[r]);
        const water = rMax - height[r];
        totalWater += water;
        steps.push({ line: 8, description: "rMax ≤ lMax. Shift right pointer to " + r + " (val " + height[r] + "). New rMax=" + rMax + ". Water at index " + r + " = " + water + ". Total trapped water = " + totalWater, state: { array: [...height], left: l, right: r, l_max: lMax, r_max: rMax, total_water: totalWater } });
      }
    }
    steps.push({ line: 11, description: "✅ Completed Trapping Rain Water. Total trapped water = " + totalWater, state: { array: [...height], total_water: totalWater } });
  }

  // ── Container With Most Water ─────────────────────────────────────────────
  else if (algorithmId === "container-with-most-water") {
    const height = input.height || [1, 8, 6, 2, 5, 4, 8, 3, 7];
    steps.push({ line: 1, description: "💧 Container With Most Water! Find two lines to trap the most water. Heights: [" + height.join(", ") + "]", state: { array: [...height] } });
    ...
    while (left < right) {
      ...
      steps.push({
        line: 4,
        description: "Check left=" + left + " (val " + height[left] + "), right=" + right + " (val " + height[right] + "). Width=" + w + ", height=" + h + " → Area=" + area + ". " + (area > oldMax ? "🎉 New max area = " + maxArea + "!" : "Keep max area = " + maxArea),
        state: { array: [...height], left, right, current_area: area, max_area: maxArea }
      });
    ...
    steps.push({ line: 8, description: "✅ Optimal Container completed! Maximum area is " + maxArea, state: { array: [...height], max_area: maxArea } });
  }

  // ── Binary Search ─────────────────────────────────────────────────────────
  else if (algorithmId === "binary") {
    const arr = input.arr || [2, 5, 8, 12, 16, 23, 38, 56, 72, 91];
    const target = input.target || 23;
    steps.push({ line: 1, description: "🔍 Start Binary Search! Target = " + target + ". Array must be sorted.", state: { array: [...arr], target } });
    let low = 0, high = arr.length - 1;
    let found = false;
    while (low <= high) {
      const mid = Math.floor((low + high) / 2);
      steps.push({ line: 3, description: "Calculate mid: index " + mid + " (value " + arr[mid] + "). Current range: [" + low + " .. " + high + "]", state: { array: [...arr], target, left: low, right: high, check_idx: mid } });
      if (arr[mid] === target) {
        steps.push({ line: 4, description: "🎯 Target found at index " + mid + "!", state: { array: [...arr], target, left: low, right: high, check_idx: mid, result: mid } });
        found = true;
        break;
      } else if (arr[mid] < target) {
        steps.push({ line: 6, description: "arr[mid] (" + arr[mid] + ") < target (" + target + "). Search right half.", state: { array: [...arr], target, left: low, right: high, check_idx: mid } });
        low = mid + 1;
      } else {
        steps.push({ line: 8, description: "arr[mid] (" + arr[mid] + ") > target (" + target + "). Search left half.", state: { array: [...arr], target, left: low, right: high, check_idx: mid } });
        high = mid - 1;
      }
    }
    if (!found) {
      steps.push({ line: 10, description: "❌ Target " + target + " not found in array.", state: { array: [...arr], target, result: -1 } });
    }
  }
